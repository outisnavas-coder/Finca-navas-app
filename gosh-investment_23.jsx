...
function CerdosModule({role,toast}){
  ...
  const updateRow=async(tabla,id,datos)=>{
    ...
    console.log("UPDATE",tabla,"id:",id,"datos:",clean);
    try{
      const res=await supabase.from(tabla).update(clean).eq("id",id).select();
      console.log("UPDATE RESULT:",res);
      if(res.error){toast(res.error.message+" (code:"+res.error.code+")","error");console.error("UPDATE ERROR:",res.error);}
      else{toast("Actualizado ✓");setModal(null);setForm({});setEditItem(null);fetchPorcino();}
    }catch(e){console.error("CATCH:",e);toast(e.message,"error");}
    finally{setSaving(false);}
  };
