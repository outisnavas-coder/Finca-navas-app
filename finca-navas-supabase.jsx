...
const SUPABASE_ANON_KEY = "sb_publishable_PNnvMXTfo9fy0vZZOvBr3g_4qPTPyYV";
...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .sv{font-family:'Playfair Display',serif;font-size:24px;font-weight:700;color:${G.deep}}
...
// ─── DASHBOARD ────────────────────────────────────────────────────────────────
function Dashboard({gastos,ingresos}){
  const [anio,setAnio]=useState(new Date().getFullYear());
  const anos=[...new Set([...gastos,...ingresos].map(x=>x.anio))].sort((a,b)=>b-a);
  const gF=gastos.filter(g=>g.anio===anio);
  const iF=ingresos.filter(i=>i.anio===anio);
  ...
  return <div>
    <div className="fl gap3 mb4 flex-wrap">
      <span style={{fontFamily:"'Playfair Display',serif",fontSize:15,color:G.deep,fontWeight:600}}>Año fiscal:</span>
      {anos.map(a=><button key={a} className={`btn ${anio===a?"btn-p":"btn-o"} btn-sm`} onClick={()=>setAnio(a)}>{a}</button>)}
    </div>
    <div className="sg">
      <div className="sc grn"><span className="si">💵</span><span className="sl">Ingresos {anio}</span><span className="sv">{fmt$(totI)}</span><span className="str">{iF.length} transacciones</span></div>
      <div className="sc"><span className="si">📤</span><span className="sl">Gastos {anio}</span><span className="sv" style={{color:G.red}}>{fmt$(totG)}</span><span className="str neg">{gF.length} registros</span></div>
...
export default function App(){
  ...
  const totG2026=gastos.filter(g=>g.anio===2026).reduce((s,g)=>s+Number(g.monto),0);
  const totI2026=ingresos.filter(i=>i.anio===2026).reduce((s,i)=>s+Number(i.monto),0);
      ...
      <div className={`main ${!sideOpen&&!mobile?"full":""}`}>
        <header className="topbar">
          ...
          <div className="fl gap3">
            <div style={{fontSize:12,color:G.g500}}>2026: <span style={{fontWeight:700,color:totI2026-totG2026>=0?G.deep:G.red}}>{fmt$(totI2026-totG2026)}</span></div>
        ...
        <main className="content">
          ...
          {loading&&<Loading/>}
          {!loading&&<>
            {page==="dashboard"&&<Dashboard gastos={gastos} ingresos={ingresos}/>}
