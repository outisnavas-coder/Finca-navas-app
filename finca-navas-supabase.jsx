...
function CerdosModule({role,toast}){
  ...
  const fetchPorcino=async()=>{
    ...
    const [c,p,m,v,vt]=await Promise.all([
      ...
      supabase.from("partos").select("*,cerdas(nombre)").order("fecha_parto",{ascending:false}),
      supabase.from("celos_montas").select("*,cerdas!celos_montas_cerda_id_fkey(nombre)").order("fecha_monta",{ascending:false}),
  ...
  // Calcular fecha estimada parto (114 días después de monta)
  const fechaParto=(fechaMonta)=>{
    if(!fechaMonta)return"-";
    const d=new Date(fechaMonta);d.setDate(d.getDate()+114);
    return d.toISOString().split("T")[0];
  };

  // días para próximo evento
  ...
  return <div>
    ...
    {/* INVENTARIO CERDAS */}
    {tab==="cerdas"&&<div className="card">
      <div className="card-h"><h3>🐷 Inventario de Animales</h3><span className="badge bg">{cerdas.filter(c=>c.estado==="Activa").length} activos</span></div>
      <div className="tw"><table>
        <thead><tr><th>Código</th><th>Nombre</th><th>Tipo</th><th>Estado</th><th>Peso</th><th>Partos</th><th>Notas</th></tr></thead>
        <tbody>{cerdas.map(c=>{
          const nPartos=partos.filter(p=>p.cerda_id===c.id).length;
          return <tr key={c.id}>
            <td style={{fontFamily:"monospace",fontSize:12}}>{c.codigo}</td>
            <td style={{fontWeight:700}}>{c.nombre}</td>
            <td><span className={`badge ${c.tipo==="Verraco"?"bb":"bg"}`}>{c.tipo}</span></td>
            <td><span className={`badge ${c.estado==="Activa"?"bg":c.estado==="Muerta"?"br":"bk"}`}>{c.estado}</span></td>
            <td>{c.peso_kg?`${c.peso_kg}kg`:"-"}</td>
            <td style={{fontWeight:600,color:G.deep}}>{nPartos}</td>
    ...
    {/* MONTAS */}
    {tab==="montas"&&<div className="card">
      <div className="card-h"><h3>❤️ Control de Celos y Montas</h3><span className="badge bg">{montas.length} registros</span></div>
      <div className="tw"><table>
        <thead><tr><th>Cerda</th><th>Fecha Monta</th><th>Parto Est.</th><th>Días para parto</th><th>Confirmado</th><th>Notas</th></tr></thead>
        <tbody>{montas.map(m=>{
          const fp=fechaParto(m.fecha_monta);
          const dias=diasPara(fp);
          return <tr key={m.id}>
            <td style={{fontWeight:700}}>{m.cerdas?.nombre||"-"}</td>
            <td>{m.fecha_monta}</td>
            <td style={{fontWeight:600,color:G.gold}}>{fp}</td>
    ...
    {/* PARTOS */}
    {tab==="partos"&&<div className="card">
      <div className="card-h"><h3>🐣 Registro de Partos</h3><span className="badge bg">{totalLechones} lechones totales</span></div>
      <div className="tw"><table>
        <thead><tr><th>Cerda</th><th>Fecha Parto</th><th>Vivos</th><th>Muertos</th><th>Total</th><th>Notas</th></tr></thead>
        <tbody>{partos.map(p=><tr key={p.id}>
          <td style={{fontWeight:700}}>{p.cerdas?.nombre||"-"}</td>
          <td>{p.fecha_parto}</td>
    ...
    {/* VACUNAS */}
    {tab==="vacunas"&&<div className="card">
      <div className="card-h"><h3>💉 Registro de Vacunas</h3></div>
      <div className="tw"><table>
        <thead><tr><th>Animal</th><th>Vacuna</th><th>Fecha</th><th>Próxima dosis</th><th>Veterinario</th><th>Notas</th></tr></thead>
        <tbody>{vacunas.map(v=>{
          const dias=diasPara(v.proxima_dosis);
          return <tr key={v.id}>
            <td style={{fontWeight:700}}>{v.cerdas?.nombre||"-"}</td>
            <td><span className="badge bb">{v.vacuna}</span></td>
            <td>{v.fecha}</td>
            <td>{v.proxima_dosis?<span style={{color:dias!==null&&dias<7?G.red:G.g700}}>{v.proxima_dosis}{dias!==null&&dias<7&&" ⚠"}</span>:"-"}</td>
      ...
      <div className="card">
        ...
        <div className="tw"><table>
          ...
          <tbody>{ventas.map(v=><tr key={v.id}>
            ...
            <td>{v.comprador||"-"}</td>
            <td><span className="badge bk">{v.forma_pago||"-"}</span></td>
            <td style={{fontSize:12,color:G.g500}}>{v.notas||"-"}</td>
          </tr>)}</tbody>
        </table></div>
      </div>
    ...
    <div className="mo" style={{display:modal==="nueva_cerda"?"flex":"none"}} onClick={()=>setModal(null)}>
      <div className="md" onClick={e=>e.stopPropagation()}>
        ...
        <div className="fg">
          ...
          <div className="fgrp"><label>Estado</label><select value={form.estado||"Activa"} onChange={e=>setForm(f=>({...f,estado:e.target.value}))}>{ESTADOS.map(s=><option key={s}>{s}</option>)}</select></div>
        ...
        <div className="fl gap2 mt4"><button className="btn btn-p" disabled={saving} onClick={()=>save("cerdas",{nombre:form.nombre,codigo:form.codigo,tipo:form.tipo||"Madre",estado:form.estado||"Activa",peso_kg:form.peso_kg?Number(form.peso_kg):null,fecha_nacimiento:form.fecha_nacimiento||null,notas:form.notas})}>{saving?"Guardando...":"Guardar"}</button><button className="btn btn-o" onClick={()=>setModal(null)}>Cancelar</button></div>
      </div>
    </div>
    ...
    <div className="mo" style={{display:modal==="nueva_monta"?"flex":"none"}} onClick={()=>setModal(null)}>
      <div className="md" onClick={e=>e.stopPropagation()}>
        ...
        <p style={{fontSize:12,color:G.gold,marginTop:10}}>📅 Parto estimado: <strong>{form.fecha_monta?fechaParto(form.fecha_monta):"-"}</strong> (114 días)</p>
    ...
    <div className="mo" style={{display:modal==="nueva_vacuna"?"flex":"none"}} onClick={()=>setModal(null)}>
      <div className="md" onClick={e=>e.stopPropagation()}>
        ...
        <div className="fg">
          ...
          <div className="fgrp"><label>Dosis</label><input value={form.dosis||""} onChange={e=>setForm(f=>({...f,dosis:e.target.value}))}/></div>
        ...
        <div className="fl gap2 mt4"><button className="btn btn-p" disabled={saving} onClick={()=>save("vacunas_cerdas",{cerda_id:form.cerda_id,vacuna:form.vacuna,fecha:form.fecha,proxima_dosis:form.proxima_dosis||null,dosis:form.dosis,veterinario:form.veterinario,notas:form.notas})}>{saving?"Guardando...":"Guardar"}</button><button className="btn btn-o" onClick={()=>setModal(null)}>Cancelar</button></div>
      </div>
    </div>
    ...
    <div className="mo" style={{display:modal==="nueva_venta"?"flex":"none"}} onClick={()=>setModal(null)}>
      <div className="md" onClick={e=>e.stopPropagation()}>
        ...
        {form.cantidad&&form.precio_unit&&<p style={{fontSize:13,color:G.deep,fontWeight:700,marginTop:10}}>Total estimado: {fmt$(Number(form.cantidad)*Number(form.precio_unit))}</p>}
