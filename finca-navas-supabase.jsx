...
function CerdosModule({role,toast}){
  ...
  const updateRow=async(tabla,id,datos)=>{
    ...
    setSaving(false);
    if(error)toast(error.message,"error");
    else{toast("Actualizado ✓");setModal(null);setForm({});setEditItem(null);fetchPorcino();}
  };
  ...
  const buildTimelineData=()=>{
    ...
    const rows=cerdas.filter(c=>c.tipo==="Madre").map(c=>{
  ...
  return <div>
    ...
    {(modal==="edit_monta"||modal==="new_monta")&&<div className="mo" onClick={()=>setModal(null)}><div className="md" onClick={e=>e.stopPropagation()}>
      ...
      <div className="fl gap2 mt4">
        {modal==="edit_monta"
          ?<button className="btn btn-p" disabled={saving} onClick={()=>updateRow("celos_montas",editItem.id,{cerda_id:form.cerda_id,fecha_monta:form.fecha_monta,fecha_celo:form.fecha_celo||null,verraco_id:form.verraco_id||null,confirmado:form.confirmado||false,notas:form.notas})}>{saving?"Guardando...":"Guardar cambios"}</button>
    ...
    {(modal==="edit_parto"||modal==="new_parto")&&<div className="mo" onClick={()=>setModal(null)}><div className="md" onClick={e=>e.stopPropagation()}>
      ...
      <div className="fl gap2 mt4">
        {modal==="edit_parto"
          ?<button className="btn btn-p" disabled={saving} onClick={()=>updateRow("partos",editItem.id,{cerda_id:form.cerda_id,fecha_parto:form.fecha_parto,lechones_vivos:Number(form.lechones_vivos||0),lechones_muertos:Number(form.lechones_muertos||0),notas:form.notas})}>{saving?"Guardando...":"Guardar cambios"}</button>
    ...
    {(modal==="edit_vacuna"||modal==="new_vacuna")&&<div className="mo" onClick={()=>setModal(null)}><div className="md" onClick={e=>e.stopPropagation()}>
      ...
      <div className="fl gap2 mt4">
        {modal==="edit_vacuna"
          ?<button className="btn btn-p" disabled={saving} onClick={()=>updateRow("vacunas_cerdas",editItem.id,{cerda_id:form.cerda_id,vacuna:form.vacuna,fecha:form.fecha,proxima_dosis:form.proxima_dosis||null,veterinario:form.veterinario,notas:form.notas})}>{saving?"Guardando...":"Guardar cambios"}</button>
    ...
    {(modal==="edit_venta"||modal==="new_venta")&&<div className="mo" onClick={()=>setModal(null)}><div className="md" onClick={e=>e.stopPropagation()}>
      ...
      <div className="fl gap2 mt4">
        {modal==="edit_venta"
          ?<button className="btn btn-p" disabled={saving} onClick={()=>updateRow("ventas_lechones",editItem.id,{fecha:form.fecha,cantidad:Number(form.cantidad),precio_unit:Number(form.precio_unit),comprador:form.comprador,forma_pago:form.forma_pago,notas:form.notas})}>{saving?"Guardando...":"Guardar cambios"}</button>
