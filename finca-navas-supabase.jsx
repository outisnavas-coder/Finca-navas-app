...
function CerdosModule({role,toast}){
  const [tab,setTab]=useState("cerdas");
  ...
  const [form,setForm]=useState({});
  const [saving,setSaving]=useState(false);
  const [editCerda,setEditCerda]=useState(null);
  ...
  const fechaParto=(f)=>{if(!f)return"-";const d=new Date(f);d.setDate(d.getDate()+114);return d.toISOString().split("T")[0];};
  const diasPara=(f)=>{if(!f)return null;return Math.ceil((new Date(f)-new Date())/(1000*60*60*24));};

  const save=async(tabla,datos)=>{
    setSaving(true);
    const {error}=await supabase.from(tabla).insert(datos);
    setSaving(false);
    if(error)toast(error.message,"error");
    else{toast("Guardado ✓");setModal(null);setForm({});fetchPorcino();}
  };

  const updateCerda=async()=>{
    setSaving(true);
    const {error}=await supabase.from("cerdas").update({
      estado:form.estado,
      peso_kg:form.peso_kg?Number(form.peso_kg):editCerda.peso_kg,
      notas:form.notas||editCerda.notas
    }).eq("id",editCerda.id);
    setSaving(false);
    if(error)toast(error.message,"error");
    else{toast("Actualizado ✓");setModal(null);setForm({});setEditCerda(null);fetchPorcino();}
  };
  ...
  // Group montas by cerda - latest first
  const montasByCerda={};
  montas.forEach(m=>{
    const nombre=m.cerdas?.nombre||"?";
    if(!montasByCerda[nombre])montasByCerda[nombre]=[];
    montasByCerda[nombre].push(m);
  });

  // Group partos by cerda
  const partosByCerda={};
  partos.forEach(p=>{
    const nombre=p.cerdas?.nombre||"?";
    if(!partosByCerda[nombre])partosByCerda[nombre]=[];
    partosByCerda[nombre].push(p);
  });

  // Group vacunas by cerda
  const vacunasByCerda={};
  vacunas.forEach(v=>{
    const nombre=v.cerdas?.nombre||"?";
    if(!vacunasByCerda[nombre])vacunasByCerda[nombre]=[];
    vacunasByCerda[nombre].push(v);
  });

  const estadoBadge=(e)=>{
    if(e==="Activa")return "bg";
    if(e==="Muerta")return "br";
    if(e==="Vendida")return "bo";
    return "bk";
  };
  ...
  return <div>
    ...
    <div className="fl mb4" style={{flexWrap:"wrap",gap:4}}>
      <div className="tabs" style={{marginBottom:0,flex:1}}>
        {[["cerdas","🐷 Inventario"],["timeline","📅 Timeline"],["montas","❤️ Montas"],["partos","🐣 Partos"],["vacunas","💉 Vacunas"],["ventas","🤝 Ventas"]].map(([t,l])=>
          <button key={t} className={`tab ${tab===t?"active":""}`} onClick={()=>setTab(t)}>{l}</button>
        )}
      </div>
      {role==="admin"&&<button className="btn btn-p btn-sm" style={{marginLeft:8}} onClick={()=>{
        setModal(tab==="cerdas"?"nueva_cerda":tab==="montas"?"nueva_monta":tab==="partos"?"nuevo_parto":tab==="vacunas"?"nueva_vacuna":"nueva_venta");
        setForm({});
      }}>+ Agregar</button>}
    </div>

    {/* INVENTARIO — agrupado por Madres y Verracos */}
    {tab==="cerdas"&&<div>
      {/* MADRES ACTIVAS */}
      <div className="card mb4">
        <div className="card-h"><h3>🐷 Cerdas Madres</h3><span className="badge bg">{cerdas.filter(c=>c.tipo==="Madre"&&c.estado==="Activa").length} activas</span></div>
        <div className="tw"><table>
          <thead><tr><th>Código</th><th>Nombre</th><th>Estado</th><th>Peso</th><th>Partos</th><th>Notas</th>{role==="admin"&&<th></th>}</tr></thead>
          <tbody>{cerdas.filter(c=>c.tipo==="Madre"&&c.estado==="Activa").map(c=>{
            const nP=partos.filter(p=>p.cerda_id===c.id).length;
            return <tr key={c.id}>
              ...
              <td style={{fontWeight:600,color:G.deep}}>{nP}</td>
              <td style={{fontSize:12,color:G.g500}}>{c.notas||"-"}</td>
              {role==="admin"&&<td><button className="btn btn-sm btn-o" onClick={()=>{setEditCerda(c);setForm({estado:c.estado,peso_kg:c.peso_kg||"",notas:c.notas||""});setModal("editar_cerda");}}>✏ Editar</button></td>}
            </tr>;
          })}</tbody>
        </table></div>
      </div>

      {/* HISTORIAL — inactivas/vendidas/muertas */}
      {cerdas.filter(c=>c.tipo==="Madre"&&c.estado!=="Activa").length>0&&<div className="card mb4">
        <div className="card-h"><h3>📋 Historial Madres</h3><span className="badge bk">{cerdas.filter(c=>c.tipo==="Madre"&&c.estado!=="Activa").length} registros</span></div>
        <div className="tw"><table>
          <thead><tr><th>Código</th><th>Nombre</th><th>Estado</th><th>Peso</th><th>Partos</th><th>Notas</th>{role==="admin"&&<th></th>}</tr></thead>
          <tbody>{cerdas.filter(c=>c.tipo==="Madre"&&c.estado!=="Activa").map(c=>{
            const nP=partos.filter(p=>p.cerda_id===c.id).length;
            return <tr key={c.id}>
              <td style={{fontFamily:"monospace",fontSize:12}}>{c.codigo}</td>
              <td style={{fontWeight:700,color:G.g500}}>{c.nombre}</td>
              <td><span className={`badge ${estadoBadge(c.estado)}`}>{c.estado}</span></td>
              <td>{c.peso_kg?`${c.peso_kg}kg`:"-"}</td>
    ...
    {/* MONTAS — agrupadas por cerda, última primero */}
    {tab==="montas"&&<div>
      {Object.entries(montasByCerda).map(([nombre,mts])=><div key={nombre} className="card mb4">
        <div className="card-h">
          ...
          <div className="fl gap2">
            ...
            {mts[0]?.fecha_monta&&<span style={{fontSize:12,color:G.gold}}>Última: {mts[0].fecha_monta}</span>}
          </div>
        </div>
        <div className="tw"><table>
          <thead><tr><th>Fecha Monta</th><th>Parto Est.</th><th>Días restantes</th><th>Confirmado</th><th>Notas</th></tr></thead>
          <tbody>{mts.map(m=>{
            const fp=fechaParto(m.fecha_monta);
            const dias=diasPara(fp);
            return <tr key={m.id}>
              <td style={{fontWeight:600}}>{m.fecha_monta}</td>
              <td style={{color:G.gold,fontWeight:600}}>{fp}</td>
              <td>{dias!==null?<span style={{fontWeight:700,color:dias<0?G.g500:dias<14?G.red:dias<30?G.gold:G.deep}}>{dias<0?"✓ Parido":dias+" días"}</span>:"-"}</td>
              <td><span className={`badge ${m.confirmado?"bg":"bo"}`}>{m.confirmado?"✓ Sí":"Pendiente"}</span></td>
    ...
    {/* PARTOS — consolidado por cerda */}
    {tab==="partos"&&<div>
      {Object.entries(partosByCerda).sort((a,b)=>b[1][0].fecha_parto.localeCompare(a[1][0].fecha_parto)).map(([nombre,pts])=>{
        const totalV=pts.reduce((s,p)=>s+p.lechones_vivos,0);
        const totalM=pts.reduce((s,p)=>s+p.lechones_muertos,0);
        return <div key={nombre} className="card mb4">
          <div className="card-h">
            <h3>🐣 {nombre}</h3>
            <div className="fl gap2">
              <span className="badge bg">{pts.length} partos · {totalV} vivos</span>
              {totalM>0&&<span className="badge br">{totalM} muertos</span>}
            </div>
          </div>
          <div className="tw"><table>
            <thead><tr><th>Fecha</th><th>Vivos</th><th>Muertos</th><th>Total</th><th>Notas</th></tr></thead>
            <tbody>{pts.map(p=><tr key={p.id}>
              <td style={{fontWeight:600}}>{p.fecha_parto}</td>
          ...
          <div style={{padding:"10px 20px",background:G.beige,display:"flex",gap:20,fontSize:13}}>
            <span>Total partos: <strong>{pts.length}</strong></span>
            <span>Lechones vivos: <strong style={{color:G.deep}}>{totalV}</strong></span>
            <span>Promedio/parto: <strong>{(totalV/pts.length).toFixed(1)}</strong></span>
          </div>
        </div>;
      })}
    </div>}

    {/* VACUNAS — agrupadas por cerda con próxima dosis */}
    {tab==="vacunas"&&<div>
      {Object.entries(vacunasByCerda).map(([nombre,vacs])=>{
        const proxima=vacs.find(v=>v.proxima_dosis&&diasPara(v.proxima_dosis)!==null&&diasPara(v.proxima_dosis)>=0);
        const vencida=vacs.find(v=>v.proxima_dosis&&diasPara(v.proxima_dosis)!==null&&diasPara(v.proxima_dosis)<0);
        return <div key={nombre} className="card mb4">
          <div className="card-h">
            <h3>💉 {nombre}</h3>
            <div className="fl gap2">
              {proxima&&<span className="badge bo">Próx: {proxima.proxima_dosis} ({diasPara(proxima.proxima_dosis)}d)</span>}
              {vencida&&<span className="badge br">⚠ Dosis vencida</span>}
            </div>
          </div>
          <div className="tw"><table>
            <thead><tr><th>Vacuna</th><th>Fecha</th><th>Próxima Dosis</th><th>Días</th><th>Veterinario</th><th>Notas</th></tr></thead>
            <tbody>{vacs.map(v=>{
              const dias=diasPara(v.proxima_dosis);
              return <tr key={v.id}>
                <td><span className="badge bb">{v.vacuna}</span></td>
                <td>{v.fecha}</td>
                <td style={{color:dias!==null&&dias<7?G.red:G.g700,fontWeight:dias!==null&&dias<7?700:400}}>{v.proxima_dosis||"-"}</td>
    ...
    {tab==="ventas"&&<div>
      ...
      <div className="card">
        ...
        <div className="tw"><table>
          <thead><tr><th>Fecha</th><th>Cantidad</th><th>Precio/u</th><th>Total</th><th>Comprador</th><th>Pago</th><th>Notas</th></tr></thead>
          <tbody>{ventas.map(v=><tr key={v.id}>
            <td>{v.fecha}</td><td style={{fontWeight:700}}>{v.cantidad}</td>
    ...
    {tab==="timeline"&&(()=>{
      const meses=6;
      const tStart=new Date(TODAY.getFullYear(),TODAY.getMonth()-2,1);
      ...
      // Build timeline data per cerda
      const cTimeData=cerdas.filter(c=>c.tipo==="Madre").map(c=>{
        ...
        if(lastParto){
          ...
          if(descFin>TODAY){proxMonta=descFin;}else{proxMonta=addD(TODAY,7);}
        ...
        const segs=[];
        cPartos.forEach(p=>{
          const gStart=addD(p.fecha_parto,-GEST);
          const lEnd=addD(p.fecha_parto,LACT);
          const dEnd=addD(lEnd,DESC);
          const gs=pctPos(gStart,tStart,total),ge=pctPos(p.fecha_parto,tStart,total);
          const ls=ge,le=pctPos(lEnd,tStart,total);
          const ds=le,de=pctPos(dEnd,tStart,total);
          if(ge>gs)segs.push({l:gs,w:ge-gs,color:"#9FE1CB",title:`Gestación → parto ${fmtS(p.fecha_parto)}`});
          if(le>ls)segs.push({l:ls,w:le-ls,color:"#5DCAA5",title:`Lactancia hasta ${fmtS(lEnd)}`});
          if(de>ds)segs.push({l:ds,w:de-ds,color:"#AFA9EC",title:"Descanso/celo"});
        });

        // Projected segments
        if(proxMonta&&proxParto){
          const lEnd=addD(proxParto,LACT);
          const ps=pctPos(proxMonta,tStart,total),pe=pctPos(proxParto,tStart,total);
          const ls=pe,le=pctPos(lEnd,tStart,total);
          if(pe>ps)segs.push({l:ps,w:pe-ps,color:"#FAC775",proj:true,title:`Gestación proyectada → ${fmtS(proxParto)}`});
          if(le>ls)segs.push({l:ls,w:le-ls,color:"#EF9F27",proj:true,title:"Lactancia proyectada"});
        }

        // Dots
        const dots=[];
        cPartos.forEach(p=>{const x=pctPos(p.fecha_parto,tStart,total);if(x>=0&&x<=100)dots.push({x,color:"#378ADD",title:`Parto ${fmtS(p.fecha_parto)} — ${p.lechones_vivos} lechones`});});
        ...
        // Status
        let statusLabel="Activa",statusColor=G.deep,statusBg=G.pale;
        if(c.estado!=="Activa"){statusLabel=c.estado;statusColor=G.g500;statusBg=G.g100;}
        else if(proxParto){
          const d=diffD(TODAY,proxParto);
          if(d<0&&-d<LACT){statusLabel=`Lactancia D${-d}`;statusColor="#0F6E56";statusBg="#E1F5EE";}
          else if(d>=0&&d<=14){statusLabel=`Parto en ${d}d`;statusColor=G.red;statusBg=G.redL;}
          else if(d>=0&&d<=30){statusLabel=`Parto en ${d}d`;statusColor=G.gold;statusBg=G.goldL;}
          else if(d>0){statusLabel=`Parto: ${fmtS(proxParto)}`;statusColor=G.blue;statusBg=G.blueL;}
        }
        ...
        const vacC=vacunas.filter(v=>v.cerda_id===c.id&&v.proxima_dosis);
        vacC.forEach(v=>{
        ...
        return {c,segs,dots,proxParto,proxMonta,lastParto,statusLabel,statusColor,statusBg,checks,totalLech:cPartos.reduce((s,p)=>s+p.lechones_vivos,0)};
      });
      ...
      return <div>
        ...
        <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:14}}>
          {[["#9FE1CB","Gestación"],["#5DCAA5","Lactancia"],["#AFA9EC","Descanso"],["#FAC775","Proyectado"]].map(([c,l])=>
            <div key={l} style={{display:"flex",alignItems:"center",gap:5,fontSize:12,color:G.g500}}>
              <div style={{width:12,height:12,borderRadius:3,background:c,flexShrink:0}}></div>{l}
            </div>
          )}
          {[["#378ADD","Parto real"],["#639922","Monta"],["#E24B4A","Parto proy."]].map(([c,l])=>
            <div key={l} style={{display:"flex",alignItems:"center",gap:5,fontSize:12,color:G.g500}}>
              <div style={{width:10,height:10,borderRadius:"50%",background:c,flexShrink:0}}></div>{l}
            </div>
          )}
        </div>
        ...
        {/* Rows per cerda */}
        {cTimeData.map(({c,segs,dots,proxParto,lastParto,statusLabel,statusColor,statusBg,checks,totalLech})=><div key={c.id} className="card mb4" style={{overflow:"hidden"}}>
          <div style={{display:"flex",alignItems:"stretch"}}>
            {/* Label */}
            <div style={{width:88,flexShrink:0,padding:"10px 10px",borderRight:`0.5px solid ${G.beigeD}`,display:"flex",flexDirection:"column",justifyContent:"center",gap:4}}>
              <span style={{fontWeight:600,fontSize:13}}>{c.nombre}</span>
              <span style={{fontSize:10,padding:"2px 6px",borderRadius:20,background:statusBg,color:statusColor,textAlign:"center"}}>{statusLabel}</span>
            </div>
            {/* Timeline track */}
            <div style={{flex:1,padding:"10px 8px",position:"relative",overflow:"hidden"}}>
              <div style={{position:"relative",height:24,background:G.beige,borderRadius:4}}>
                {/* Segments */}
                {segs.map((s,i)=><div key={i} title={s.title} style={{position:"absolute",left:`${s.l.toFixed(1)}%`,width:`${s.w.toFixed(1)}%`,height:"100%",background:s.color,borderRadius:3,border:s.proj?"1px dashed rgba(0,0,0,.2)":"none",cursor:"default"}}></div>)}
                {/* Dots */}
                {dots.map((d,i)=><div key={i} title={d.title} style={{position:"absolute",left:`${d.x.toFixed(1)}%`,top:"50%",width:9,height:9,borderRadius:"50%",background:d.color,border:`1.5px solid ${G.white}`,transform:"translate(-50%,-50%)",zIndex:5,cursor:"default"}}></div>)}
                {/* Today line */}
              ...
              {checks.length>0&&<div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:6}}>
                {checks.map((ch,i)=><span key={i} style={{fontSize:10,padding:"2px 7px",borderRadius:20,background:ch.s==="done"?G.pale:ch.s==="alert"?G.redL:G.goldL,color:ch.s==="done"?G.deep:ch.s==="alert"?G.red:G.gold}}>{ch.s==="done"?"✓":ch.s==="alert"?"!":"○"} {ch.l}</span>)}
              </div>}
            </div>
            {/* Stats mini */}
            <div style={{width:90,flexShrink:0,padding:"10px 8px",borderLeft:`0.5px solid ${G.beigeD}`,display:"flex",flexDirection:"column",justifyContent:"center",gap:3,fontSize:11,color:G.g500}}>
              <div>Partos: <strong style={{color:G.deep}}>{partos.filter(p=>p.cerda_id===c.id).length}</strong></div>
              <div>Lechones: <strong style={{color:G.deep}}>{totalLech}</strong></div>
              {proxParto&&<div style={{color:diffD(TODAY,proxParto)<14?G.red:G.gold,fontWeight:600}}>{fmtS(proxParto)}</div>}
            </div>
          </div>
        </div>)}
      </div>;
    ...
    {/* MODAL EDITAR CERDA */}
    <div className="mo" style={{display:modal==="editar_cerda"?"flex":"none"}} onClick={()=>setModal(null)}>
      <div className="md" onClick={e=>e.stopPropagation()}>
        <div className="fb mb4"><h3>Editar: {editCerda?.nombre}</h3><button className="btn btn-o btn-sm" onClick={()=>setModal(null)}>✕</button></div>
        <div className="fg">
          ...
          <div className="fgrp"><label>Peso actual (kg)</label><input type="number" step="0.1" value={form.peso_kg||""} placeholder={editCerda?.peso_kg||"Sin registro"} onChange={e=>setForm(f=>({...f,peso_kg:e.target.value}))}/></div>
          <div className="fgrp"><label>Notas</label><input value={form.notas||""} onChange={e=>setForm(f=>({...f,notas:e.target.value}))}/></div>
        </div>
        <div className="fl gap2 mt4"><button className="btn btn-p" disabled={saving} onClick={updateCerda}>{saving?"Guardando...":"Guardar cambios"}</button><button className="btn btn-o" onClick={()=>setModal(null)}>Cancelar</button></div>
      </div>
    </div>

    {/* MODAL NUEVA CERDA */}
    <div className="mo" style={{display:modal==="nueva_cerda"?"flex":"none"}} onClick={()=>setModal(null)}>
      <div className="md" onClick={e=>e.stopPropagation()}>
        ...
        <div className="fl gap2 mt4"><button className="btn btn-p" disabled={saving} onClick={()=>save("cerdas",{nombre:form.nombre,codigo:form.codigo,tipo:form.tipo||"Madre",estado:"Activa",peso_kg:form.peso_kg?Number(form.peso_kg):null,fecha_nacimiento:form.fecha_nacimiento||null,notas:form.notas})}>{saving?"Guardando...":"Guardar"}</button><button className="btn btn-o" onClick={()=>setModal(null)}>Cancelar</button></div>
      </div>
    </div>

    {/* MODAL NUEVA MONTA */}
    <div className="mo" style={{display:modal==="nueva_monta"?"flex":"none"}} onClick={()=>setModal(null)}>
      <div className="md" onClick={e=>e.stopPropagation()}>
        <div className="fb mb4"><h3>Registrar Monta</h3><button className="btn btn-o btn-sm" onClick={()=>setModal(null)}>✕</button></div>
        <div className="fg">
          ...
          <div className="fgrp"><label>Fecha Monta</label><input type="date" value={form.fecha_monta||""} onChange={e=>setForm(f=>({...f,fecha_monta:e.target.value}))}/></div>
          <div className="fgrp"><label>Fecha Celo</label><input type="date" value={form.fecha_celo||""} onChange={e=>setForm(f=>({...f,fecha_celo:e.target.value}))}/></div>
          ...
          <div className="fgrp"><label>Confirmado</label><select value={form.confirmado||"false"} onChange={e=>setForm(f=>({...f,confirmado:e.target.value==="true"}))}><option value="false">Pendiente</option><option value="true">Confirmado</option></select></div>
          <div className="fgrp"><label>Notas</label><input value={form.notas||""} onChange={e=>setForm(f=>({...f,notas:e.target.value}))}/></div>
        </div>
        {form.fecha_monta&&<p style={{fontSize:12,color:G.gold,marginTop:10}}>📅 Parto estimado: <strong>{fechaParto(form.fecha_monta)}</strong> (114 días)</p>}
        <div className="fl gap2 mt4"><button className="btn btn-p" disabled={saving} onClick={()=>save("celos_montas",{cerda_id:form.cerda_id,fecha_monta:form.fecha_monta,fecha_celo:form.fecha_celo||null,verraco_id:form.verraco_id||null,confirmado:form.confirmado||false,notas:form.notas})}>{saving?"Guardando...":"Guardar"}</button><button className="btn btn-o" onClick={()=>setModal(null)}>Cancelar</button></div>
      </div>
    </div>

    {/* MODAL NUEVO PARTO */}
    <div className="mo" style={{display:modal==="nuevo_parto"?"flex":"none"}} onClick={()=>setModal(null)}>
      <div className="md" onClick={e=>e.stopPropagation()}>
        <div className="fb mb4"><h3>Registrar Parto</h3><button className="btn btn-o btn-sm" onClick={()=>setModal(null)}>✕</button></div>
        <div className="fg">
          <div className="fgrp"><label>Cerda</label><select value={form.cerda_id||""} onChange={e=>setForm(f=>({...f,cerda_id:e.target.value}))}><option value="">Seleccionar...</option>{madres.map(c=><option key={c.id} value={c.id}>{c.nombre}</option>)}</select></div>
          <div className="fgrp"><label>Fecha Parto</label><input type="date" value={form.fecha_parto||""} onChange={e=>setForm(f=>({...f,fecha_parto:e.target.value}))}/></div>
        ...
        <div className="fl gap2 mt4"><button className="btn btn-p" disabled={saving} onClick={()=>save("partos",{cerda_id:form.cerda_id,fecha_parto:form.fecha_parto,lechones_vivos:Number(form.lechones_vivos||0),lechones_muertos:Number(form.lechones_muertos||0),notas:form.notas})}>{saving?"Guardando...":"Guardar"}</button><button className="btn btn-o" onClick={()=>setModal(null)}>Cancelar</button></div>
      </div>
    </div>

    {/* MODAL NUEVA VACUNA */}
    <div className="mo" style={{display:modal==="nueva_vacuna"?"flex":"none"}} onClick={()=>setModal(null)}>
      <div className="md" onClick={e=>e.stopPropagation()}>
        <div className="fb mb4"><h3>Registrar Vacuna</h3><button className="btn btn-o btn-sm" onClick={()=>setModal(null)}>✕</button></div>
        <div className="fg">
          ...
          <div className="fgrp"><label>Fecha</label><input type="date" value={form.fecha||""} onChange={e=>setForm(f=>({...f,fecha:e.target.value}))}/></div>
          <div className="fgrp"><label>Próxima Dosis</label><input type="date" value={form.proxima_dosis||""} onChange={e=>setForm(f=>({...f,proxima_dosis:e.target.value}))}/></div>
        ...
        <div className="fl gap2 mt4"><button className="btn btn-p" disabled={saving} onClick={()=>save("vacunas_cerdas",{cerda_id:form.cerda_id,vacuna:form.vacuna,fecha:form.fecha,proxima_dosis:form.proxima_dosis||null,veterinario:form.veterinario,notas:form.notas})}>{saving?"Guardando...":"Guardar"}</button><button className="btn btn-o" onClick={()=>setModal(null)}>Cancelar</button></div>
      </div>
    </div>

    {/* MODAL NUEVA VENTA */}
    <div className="mo" style={{display:modal==="nueva_venta"?"flex":"none"}} onClick={()=>setModal(null)}>
      <div className="md" onClick={e=>e.stopPropagation()}>
        <div className="fb mb4"><h3>Registrar Venta de Lechones</h3><button className="btn btn-o btn-sm" onClick={()=>setModal(null)}>✕</button></div>
        <div className="fg">
          <div className="fgrp"><label>Fecha</label><input type="date" value={form.fecha||new Date().toISOString().split("T")[0]} onChange={e=>setForm(f=>({...f,fecha:e.target.value}))}/></div>
          ...
          <div className="fgrp"><label>Precio por unidad ($)</label><input type="number" step="0.01" value={form.precio_unit||""} onChange={e=>setForm(f=>({...f,precio_unit:e.target.value}))}/></div>
        ...
        <div className="fl gap2 mt4"><button className="btn btn-p" disabled={saving} onClick={()=>save("ventas_lechones",{fecha:form.fecha,cantidad:Number(form.cantidad),precio_unit:Number(form.precio_unit),comprador:form.comprador,forma_pago:form.forma_pago,notas:form.notas})}>{saving?"Guardando...":"Guardar"}</button><button className="btn btn-o" onClick={()=>setModal(null)}>Cancelar</button></div>
      </div>
    </div>
  </div>;
}
